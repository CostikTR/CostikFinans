...
function getConfigFromEnv() {
  ...
  if (missing.length) {
    console.warn('Firebase config incomplete - missing keys:', missing.map(([k]) => k).join(', '))
    console.warn('App will run in offline/guest mode. Add Firebase env vars to enable cloud features.')
    return null
  }
...
if (cfg) {
  try {
    ...
    // Use new cache API when available; fallback to enableIndexedDbPersistence
    try {
    ...
    if (typeof window !== "undefined") {
      // For legacy environments, ensure persistence; ignore failures
      try { enableIndexedDbPersistence(db!).catch(() => void 0) } catch {}
    }
