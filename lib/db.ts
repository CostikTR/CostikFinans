...
// Normalize Firestore Timestamp-like values to primitives
...
// Convert various timestamp-like inputs to Date
function toDateFromAny(v: any): Date | null {
  try {
    if (!v) return null
...
function normalizeTimestamps<T extends Record<string, any>>(doc: T): T {
  const out: any = { ...doc }
  // Heuristics: fields including 'date' -> YYYY-MM-DD, fields ending with 'At' or named 'timestamp' -> ISO string
  Object.keys(out).forEach((k) => {
    const v = out[k]
    const d = toDateFromAny(v)
    if (!d) return
    const lk = k.toLowerCase()
    if (lk === 'date' || lk.endsWith('date') || lk.includes('date')) {
      out[k] = toYMD(d)
    } else if (lk === 'timestamp' || lk.endsWith('timestamp') || lk.endsWith('at')) {
      out[k] = d.toISOString()
    }
  })
  return out
...
export async function listTransactions(userId: string): Promise<Transaction[]> {
  ...
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Transaction[]
  } catch (error) {
...
export function watchTransactions(
  ...
  return onSnapshot(
    q,
    (snap) => {
      const list = snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Transaction[]
      cb(list)
    },
    (err) => {
...
export async function listNotifications(userId: string): Promise<Notification[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Notification[]
  } catch (error) {
...
export function watchNotifications(userId: string, cb: (list: Notification[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        cb(snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Notification[])
      },
      (err) => {
...
export async function listNotes(userId: string): Promise<Note[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Note[]
  } catch (error) {
...
export function watchNotes(userId: string, cb: (list: Note[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        cb(snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Note[])
      },
      (err) => {
...
export async function listSubscriptions(userId: string): Promise<Subscription[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Subscription[]
  } catch (error) {
...
export function watchSubscriptions(userId: string, cb: (list: Subscription[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        cb(snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Subscription[])
      },
      (err) => {
...
export async function listCards(userId: string): Promise<BankCard[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as BankCard[]
  } catch (error) {
...
export function watchCards(userId: string, cb: (list: BankCard[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        cb(snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as BankCard[])
      },
      (err) => {
...
export async function listCardEntries(userId: string, cardId: string): Promise<CardEntry[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as CardEntry[]
  } catch (error) {
...
export function watchPayments(userId: string, cb: (payments: Payment[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        const list = snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as Payment[]
        cb(list)
      },
      (err) => {
...
export async function listRecurringIncomes(userId: string): Promise<RecurringIncome[]> {
  ...
  const snap = await getDocs(q)
  return snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as RecurringIncome[]
  } catch (error) {
...
export function watchRecurringIncomes(userId: string, cb: (list: RecurringIncome[]) => void): Unsubscribe | null {
  ...
  try {
    ...
    return onSnapshot(
      q,
      (snap) => {
        cb(snap.docs.map((d) => normalizeTimestamps({ ...(d.data() as any), id: d.id })) as RecurringIncome[])
      },
      (err) => {
