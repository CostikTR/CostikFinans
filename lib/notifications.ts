...
export class NotificationManager {
  ...
  async sendEmailNotification(
    ...
    try {
      ...
      if (!user || !userEmail) {
        console.log('Kullanıcı bilgileri eksik')
        return
      }

      // Firebase Functions'ı çağır
      const functions = getFunctions()
      const sendEmailNotificationFn = httpsCallable(functions, 'sendEmailNotification')
      
      const emailData = {
        userId: user.uid,
        userEmail: userEmail,
        userName: userName || user.displayName || 'Değerli Kullanıcı',
        notificationType,
        data,
        priority
      }

      const result = await sendEmailNotificationFn(emailData)
      console.log('E-posta bildirimi gönderildi:', result.data)

    } catch (error) {
      console.error('E-posta bildirimi gönderme hatası:', error)
      // E-posta gönderilemezse browser bildirimi gönder
      const notification = this.createNotification(notificationType, data, priority)
  ...
  // Bildirim oluştur ve kaydet (hem browser hem e-posta)
  async createNotificationWithEmail(
    ...
    const notification = this.createNotification(type, data, priority, scheduledFor)

    // E-posta bildirimi de gönder (eğer ayarlar aktifse)
    if (userEmail) {
      try {
        await this.sendEmailNotification(type, data, userEmail, userName, priority)
      } catch (error) {
