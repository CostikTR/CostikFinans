...
import { checkAdminStatus } from "@/lib/admin-helper"
...
export default function FinanceDashboard() {
  ...
  const onAdd = async (t: Transaction) => {
    ...
    // Bildirim oluştur
    createNotificationWithEmail('transaction_alert', {
      type: t.type,
    ...
    const run = async () => {
      ...
      try {
        ...
        for (const r of list) {
          ...
          if (!due) continue
          const d0 = new Date(due.getFullYear(), due.getMonth(), due.getDate())
          // Seçili tarihe göre yükleme yap - tarih geçmişse veya bugünse yükle
          if (d0 <= t0) {
            // İşlemi seçili tarihte oluştur, bugüne değil
            const txn = { type: "gelir" as const, amount: Number(r.amount) || 0, description: r.description, category: r.category, date: d0.toISOString().slice(0,10) }
          ...
          <main className="container mx-auto px-4 py-8 space-y-8">
            <FadeIn>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-4">
                {/* Admin Panel Butonu - Sadece yetkili kullanıcılara göster */}
                {isAdmin && (
                  <ScalePress>
                    <Button 
